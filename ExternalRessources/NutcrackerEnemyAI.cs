...
if (__rpc_exec_stage == __RpcExecStage.Server && (networkManager.IsServer  networkManager.IsHost))
	...
	gameObject.GetComponentNetworkObject().Spawn();
	GrabGun(gameObject);
	randomSeedNumber = UnityEngine.Random.Range(0, 10000);
	InitializeNutcrackerValuesClientRpc(randomSeedNumber, gameObject.GetComponentNetworkObject());
}
	}
...
{
	gun = gunObject.GetComponentShotgunItem();
	if (gun == null)
	{
...
private void SpawnShotgunShells()
{
	...
	obj.GetComponentGrabbableObject().fallTime = 0f;
	obj.GetComponentNetworkObject().Spawn();
}
		}
	}
	...
	private bool GrabGunIfNotHolding()
{
	...
	{
		gun = networkObject.gameObject.GetComponentShotgunItem();
